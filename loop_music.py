...
import main
from mood import MoodConfig, MoodStateHolder, mood_worker
from music_engine import MusicConfig, music_engine_loop
...
# Mood detection / music mode configuration
MOOD_ENABLED = True
MOOD_POLL_INTERVAL_SECONDS = 30.0         # analyze mood every 30 seconds
MOOD_PRIMARY_WINDOW_SECONDS = 30.0        # last 30s drives mood
MOOD_CONTEXT_WINDOW_SECONDS = 120.0       # last 2 minutes as context
MOOD_MTM_TAIL = 5
MOOD_HISTORY_LEN = 3
MOOD_FLIP_PATIENCE = 1
MOOD_MIN_DURATION_SECONDS = 5.0
MOOD_CONFIDENCE_GATE = 0.6
MOOD_MODEL_NAME = os.getenv("MOOD_MODEL", MODEL_NAME)
MUSIC_ENABLED = True
MUSIC_MIDI_PORT = os.getenv("MUSIC_MIDI_PORT")
MUSIC_SOUNDFONT = os.getenv("MUSIC_SOUNDFONT")  # optional .sf2 path for in-process synth
# Fallback: use bundled piano.sf2 if env not set and file exists
if not MUSIC_SOUNDFONT:
    default_sf2 = pathlib.Path(__file__).with_name("piano.sf2")
    if default_sf2.exists():
        MUSIC_SOUNDFONT = str(default_sf2)
...
SUMMARY_PROMPT = (
    ...
    "Use plain, concrete language; include specific details; avoid fluff. "
    "If empty or noise, return 'no speech'.\n\n"
    "Transcript:\n{transcript}\n\nSummary:"
)

LTM_PROMPT = (
    "You maintain long-term conversation memory for this user.\n\n"
    "Your job:\n"
    "• Treat CURRENT bullets as durable patterns that should be preserved unless contradicted by strong new evidence.\n"
    "• Use RECENT summaries only to update, refine, or very selectively add patterns that appear repeatedly or carry significant weight for the user.\n"
    "• NEVER overwrite CURRENT bullets with short-lived or one-time themes.\n"
    "• Only store patterns that are persistent, repeated, or core to the user's interests, projects, goals, or recurring issues.\n"
    "• Merge similar items instead of duplicating them.\n"
    "• Remove items only if RECENT strongly contradicts them.\n"
    "• Keep bullets concrete and specific to recurring topics (e.g., ongoing project X, repeated concern Y), avoid generic advice.\n"
...
TITLE_PROMPT = (
    ...
    "title (max 5 words) that mentions the specific issue or topic "
...
mtm = deque(maxlen=MTM_MAX_LEN)  # list of {"ts", "summary", "title"}
ltm = []  # list of bullet strings
chunk_buffer = []  # raw strings collected since last summary
log_lock = threading.Lock()
start_time = time.time()
warmup_summaries = []
mood_state_holder = MoodStateHolder()
mood_state_holder.set(
    {"timestamp": start_time, "mood": "neutral", "energy": 1, "confidence": 0.5}
)
...
def call_chat(prompt: str, model: str = None) -> str:
...
def build_stm_block():
    """
    Construct a short text block from STM entries for prompting or inspection.
    """
    with memory_lock:
        entries = list(stm)
    lines = []
...
def get_recent_stm_text(window_seconds: float):
    """
    Return raw STM text within the given window.
    """
    cutoff = time.time() - window_seconds
    with memory_lock:
        return [item["text"] for item in stm if item["ts"] >= cutoff]


def get_recent_mtm_summaries(limit: int):
    """
    Return the most recent MTM summaries (no titles) for mood context.
    """
    with memory_lock:
        return [item["summary"] for item in list(mtm)[-limit:] if item.get("summary")]
...
def loop():
    """
    Conversation memory loop: builds STM/MTM/LTM while main.py captures audio.
    """
    print(
        f"Conversation memory loop using {model_name} via {API_MODE} | "
        f"STM window {STM_WINDOW_SECONDS//60}m | MTM max {MTM_MAX_LEN} | "
        f"LTM refresh every {LTM_REFRESH_CHUNKS} chunks"
    )
    if MOOD_ENABLED:
        print(
            f"Mood detection enabled | primary {MOOD_PRIMARY_WINDOW_SECONDS}s | "
            f"context {MOOD_CONTEXT_WINDOW_SECONDS}s | poll {MOOD_POLL_INTERVAL_SECONDS}s"
        )
    else:
        print("Mood detection disabled")
    if MUSIC_ENABLED:
        print(
            f"Music engine enabled | MIDI port: {MUSIC_MIDI_PORT or 'default'} | "
            f"Soundfont: {MUSIC_SOUNDFONT or 'none'}"
        )
    else:
        print("Music engine disabled")

    mood_cfg = MoodConfig(
        poll_interval_s=MOOD_POLL_INTERVAL_SECONDS,
        primary_window_s=MOOD_PRIMARY_WINDOW_SECONDS,
        context_window_s=MOOD_CONTEXT_WINDOW_SECONDS,
        mtm_tail=MOOD_MTM_TAIL,
        history_len=MOOD_HISTORY_LEN,
        flip_patience=MOOD_FLIP_PATIENCE,
        min_duration_s=MOOD_MIN_DURATION_SECONDS,
        confidence_gate=MOOD_CONFIDENCE_GATE,
        model_name=MOOD_MODEL_NAME,
    )
    music_cfg = MusicConfig(
        enabled=MUSIC_ENABLED,
        midi_port=MUSIC_MIDI_PORT,
        soundfont_path=MUSIC_SOUNDFONT,
    )
    ...
    if MOOD_ENABLED:
        threading.Thread(
            ...
            args=(
                get_recent_stm_text,
                get_recent_mtm_summaries,
                mood_state_holder,
                mood_cfg,
                lambda prompt: call_chat(prompt, model=MOOD_MODEL_NAME),
            ),
            daemon=True,
        ).start()
    if MUSIC_ENABLED:
        threading.Thread(
            target=music_engine_loop,
            args=(mood_state_holder, music_cfg),
            daemon=True,
        ).start()
